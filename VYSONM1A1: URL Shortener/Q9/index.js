...
const db = new sqlite3.Database("url_shortener10M.db", (err) => {
    if (err) {
        throw new Error(err);
    }
        ...
        db.serialize(() => {
            ...
            for (let i = 0; i < rowCount; i++) {
                ...
                db.run(query, [original_url, short_code], (error) => {
                    if (error) throw error;
                    if (i === rowCount - 1) {
                        console.log(`Time taken to insert ${rowCount} URLs into the table is ${Date.now() - startTime} secs`)
                    }
                })
            }
