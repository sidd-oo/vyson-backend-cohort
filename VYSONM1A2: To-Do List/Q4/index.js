...
const runScript = () => {
    db.serialize(async () => {
        try {
            ...
            // Fetch all TODOs for a specific user, ordered by creation date
            const fetchTODOByUserQuery = `SELECT users.name, users.email, users.created_at, todos.title, todos.is_completed  
                                          FROM todos 
                                          INNER JOIN users
                                          ON todos.user_id = users.id
                                          WHERE users.name = "Sonnu" 
                                          ORDER BY todos.created_at ASC`;
            const result = await asyncAllQuery(db, fetchTODOByUserQuery);
